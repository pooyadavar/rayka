  ...
  useEffect(() => {
    ...
    for (let x = 0; x < width; x++) {
      for (let z = 0; z < depth; z++) {
        ...
        const pY =
          Math.sin(pX * 0.05) * Math.cos(pZ * 0.05) * 10 +
          Math.sin(pZ * 0.1) * 5;
    ...
    const animate = () => {
      ...
      mouseRotX += (targetY - mouseRotX) * 0.05;
      mouseRotY += (targetX - mouseRotY) * 0.05;

      // Continuous slow rotation + Mouse interaction
      baseRotationY += 0.0015;
      pointsMesh.rotation.y = baseRotationY + mouseRotY;
      pointsMesh.rotation.x = mouseRotX;

      const posArray = pointsMesh.geometry.attributes.position
        .array as Float32Array;
      for (let i = 0; i < count; i++) {
        const x = posArray[i * 3];
        // Complex wave motion
        const wave =
          Math.sin(x * 0.05 + elapsedTime * 0.6) * 3 +
          Math.cos(x * 0.03 + elapsedTime * 0.4) * 2;
  ...
  return (
    <Box
      sx={{
        ...
        background:
          "radial-gradient(circle at 50% 50%, #ffffff 0%, #f0f9ff 60%, #e0f2fe 100%)", // Modern radial gradient
      }}
    >
      <div
        ref={mountRef}
      ...
      >
        {" "}
        {/* Adjust mt for overall content position */}
        <Box sx={{ maxWidth: 700, pointerEvents: "auto" }}>
          ...
          >
            ...
            >
              ...
              >
                {" "}
                {/* Smaller and tighter letter-spacing */}
            ...
            >
              {" "}
              {/* Adjusted to h6, which is smaller than h5 */}
